...
import { analyzeProject, ANALYSIS_STEPS, generateDeterministicReadme, scoreReadme, saveReadmeToProject, saveAssetsToProject, analyzeGitHubRepo, generateChangelog, suggestCommitMessage, createProvider, AVAILABLE_PROVIDERS, } from "@readmeforge/core";
loadEnvFile();
...
app.post("/api/generate", async (req, res) => {
    const { project, options, providerId, providerConfig } = req.body;
    try {
...
app.post("/api/improve", async (req, res) => {
    const { current, project, weakAreas, providerId, providerConfig } = req.body;
    try {
        const provider = createProvider(providerId ?? "none", providerConfig ?? {});
        const available = await provider.isAvailable();
        if (!available) {
...
app.post("/api/save-readme", async (req, res) => {
    const { targetDir, content, overwrite } = req.body;
    try {
...
app.post("/api/save-assets", async (req, res) => {
    const { targetDir, images } = req.body;
    try {
